...
class TetrisGame:
    ...
    def spawn_piece(self, increment_count = True):
        """Spawns a new piece from the next queue, ensuring the queue remains filled."""

        # Reset flags to False when a new piece is spawned.
        self.wall_kick_5_used = False 
        self.hold_used = False
    ...
    def get_all_viable_hard_drops(self, weights = None):
        ...
        ### This function is horribly under-optimized, and takes literal hundreds of lines more than
        ### it probably needs. That said, I don't have the luxury of the time it would take to refactor it,
        ### and so I'm prioritizing function above all else.
        ...
        original_grid = self.grid # For lookaheads of depth > 1
        original_piece_type = self.current_piece_type # Same deal here
        ...
        # Remember the states of these before calculating at the next depth.
        depth_2_next_queue = self.next_queue
        depth_2_held_piece = self.held_piece

        # Calculate top-scoring keys at a beam width of N = 8
        top_depth_1_heuristics = dict(heapq.nlargest(8, drop_heuristics.items(), key=lambda x: x[1])) # Dict of top N heuristics
        top_depth_1_grids = {key: viable_drops[key] for key in top_depth_1_heuristics} # Grids of top N

        # Top epth-2 dictionaries:
        top_depth_2_heuristics = {}
        top_depth_2_grids = {}

        for depth_1_drop in top_depth_1_heuristics:
            _, _, hold = depth_1_drop
            if hold:
                self.hold_piece()
                self.hold_used = False

            # Reset for each depth-1 drop
            depth_2_drops = {}
            depth_2_heuristics = {}

            self.grid = top_depth_1_grids[depth_1_drop]
            self.current_piece_type, self.current_piece, self.current_rotation = self.spawn_piece(increment_count = False) 

            # Depth-2 non-held piece
            for rotation in rotations:
                ...
                if self.current_piece_type in checks[rotation]:
                    ...
                    if self.is_valid_position([(r, c - 1) for r, c in active_piece]):
                        ...
                        # Save the result
                        depth_2_drops[(-1, rotation, False, depth_1_drop)] = grid_copy
                        depth_2_heuristics[(-1, rotation, False, depth_1_drop)] = self.evaluate_heuristics(grid_copy, weights)
                        ...
                        if self.is_valid_position([(r, c - 2) for r, c in active_piece]):
                            ...
                            # Save the result
                            depth_2_drops[(-2, rotation, False, depth_1_drop)] = grid_copy
                            depth_2_heuristics[(-2, rotation, False, depth_1_drop)] = self.evaluate_heuristics(grid_copy, weights)
                            ...
                            if self.is_valid_position([(r, c - 3) for r, c in active_piece]):
                                ...
                                # Save the result
                                depth_2_drops[(-3, rotation, False, depth_1_drop)] = grid_copy
                                depth_2_heuristics[(-3, rotation, False, depth_1_drop)] = self.evaluate_heuristics(grid_copy, weights)
                                ...
                                if self.is_valid_position([(r, c - 4) for r, c in active_piece]):
                                    ...
                                    # Save the result
                                    depth_2_drops[(-4, rotation, False, depth_1_drop)] = grid_copy
                                    depth_2_heuristics[(-4, rotation, False, depth_1_drop)] = self.evaluate_heuristics(grid_copy, weights)
                            ...
                            if self.is_valid_position([(r, c + 3) for r, c in active_piece]):
                                ...
                                if self.is_valid_position([(r, c + 4) for r, c in active_piece]):
                                    ...
                                    if self.is_valid_position([(r, c + 5) for r, c in active_piece]):
                                        ...
                                        # Save the result
                                        depth_2_drops[(5, rotation, False, depth_1_drop)] = grid_copy
                                        depth_2_heuristics[(5, rotation, False, depth_1_drop)] = self.evaluate_heuristics(grid_copy, weights)
                    ...
                    # Save the result
                    depth_2_drops[(0, rotation, False, depth_1_drop)] = grid_copy
                    depth_2_heuristics[(0, rotation, False, depth_1_drop)] = self.evaluate_heuristics(grid_copy, weights)
                ...
                for rotation in rotations:
                    ...
                    if self.current_piece_type in checks[rotation]:
                        ...
                        if self.is_valid_position([(r, c - 1) for r, c in active_piece]):
                            ...
                            # Save the result
                            depth_2_drops[(-1, rotation, True, depth_1_drop)] = grid_copy
                            depth_2_heuristics[(-1, rotation, True, depth_1_drop)] = self.evaluate_heuristics(grid_copy, weights)
                            ...
                            if self.is_valid_position([(r, c - 2) for r, c in active_piece]):
                                ...
                                # Save the result
                                depth_2_drops[(-2, rotation, True, depth_1_drop)] = grid_copy
                                depth_2_heuristics[(-2, rotation, True, depth_1_drop)] = self.evaluate_heuristics(grid_copy, weights)
                                ...
                                if self.is_valid_position([(r, c - 3) for r, c in active_piece]):
                                    ...
                                    # Save the result
                                    depth_2_drops[(-3, rotation, True, depth_1_drop)] = grid_copy
                                    depth_2_heuristics[(-3, rotation, True, depth_1_drop)] = self.evaluate_heuristics(grid_copy, weights)
                                    ...
                                    if self.is_valid_position([(r, c - 4) for r, c in active_piece]):
                                        ...
                                        # Save the result
                                        depth_2_drops[(-4, rotation, True, depth_1_drop)] = grid_copy
                                        depth_2_heuristics[(-4, rotation, True, depth_1_drop)] = self.evaluate_heuristics(grid_copy, weights)
                                ...
                                if self.is_valid_position([(r, c + 3) for r, c in active_piece]):
                                    ...
                                    if self.is_valid_position([(r, c + 4) for r, c in active_piece]):
                                        ...
                                        if self.is_valid_position([(r, c + 5) for r, c in active_piece]):
                                            ...
                                            # Save the result
                                            depth_2_drops[(5, rotation, True, depth_1_drop)] = grid_copy
                                            depth_2_heuristics[(5, rotation, True, depth_1_drop)] = self.evaluate_heuristics(grid_copy, weights)
                        ...
                        # Save the result
                        depth_2_drops[(0, rotation, True, depth_1_drop)] = grid_copy
                        depth_2_heuristics[(0, rotation, True, depth_1_drop)] = self.evaluate_heuristics(grid_copy, weights)
            ...
            # Calculate top-scoring keys at a beam width of N = 8
            depth_2_heuristics = dict(heapq.nlargest(8, depth_2_heuristics.items(), key=lambda x: x[1])) # Dict of top N heuristics
            depth_2_grids = {key: depth_2_drops[key] for key in depth_2_heuristics} # Grids of top N

            # Append to top depth-2 dictionaries
            top_depth_2_heuristics |= depth_2_heuristics
            top_depth_2_grids |= depth_2_grids

            # Reset initial held piece and next queue for each iteration of the loop
            self.next_queue = depth_2_next_queue
            self.held_piece = depth_2_held_piece



        return top_depth_1_grids, top_depth_1_heuristics, top_depth_2_grids, top_depth_2_heuristics
